...
The `niceRplots` package contains four main functions. These are summarised below:
...
Detailed information on each of these functions can be found by typing `?function_name()` into the R console e.g. `?nice_gg_theme()`

The `niceRplots` package also contains three data objects which have been included as examples for use in the present cookbook. These are summarised as below:

* `anticoag_df` - data frame showing monthly primary care dispensing of 5 anticoagulant medicines for all 106 sub-ICB locations in England between October 2017 and September 2022.
* `sub_icb_shapes_2022` - Data frame containing the April 2022 polygon boundaries for all 106 sub-ICB location in England. 
* `england_shape_2022` -Single polygon containing the boundary for England, created by merging the polygons for all sub_ICB locations.

Similarly, further information on each of these data objects can be found by typing `?name` into the R console e.g. `?anticoag_df`
