...
nice_pal <- nice_colours[c("bold_teal_100", "natural_tan_100", 
...
iris_bar_formatted <- ggplot(iris_bar_df) + 
  ...
  geom_col(aes(x = Species, y = Sepal_Width), fill = nice_colours[["bold_teal_100"]], 
           colour = "#FFFFFF", linewidth = 0.2)+
  # Add baseline to x axis
  geom_hline(yintercept = 0, linewidth = 1, colour = nice_colours[["black_100"]]) +
...
nice_hist <- iris %>%
  ...
  geom_histogram(aes(x = Sepal.Width, fill = Species), 
                 binwidth = 0.2, linewidth = 0.2, color = nice_colours[["white_100"]]) +
...
bar_chart <- bar_df %>%
  ...
  geom_col(fill = nice_colours[["bold_teal_100"]], 
           colour = nice_colours[["white_100"]], linewidth = 0.2) +
  # Add baseline to x axis
  geom_hline(yintercept = 0, linewidth = 1, colour = nice_colours[["black_100"]]) +
  # set y-axis limits and remove padding
  ...
  theme(axis.title.x = ggplot2::element_blank()) +
  # Add labels
  labs(title = "Apixaban was the most prescribed anticoagulant in 2021",
       subtitle = "Total anticoagulant medicines dispensed in primary care in England, 2020",
...
line_chart <- line_df %>%
  ...
  geom_line(linewidth = 0.8, colour = nice_colours[["bold_teal_100"]]) +
  geom_point(size = 1.5, colour = nice_colours[["bold_teal_100"]]) +
...
scatter_chart <- scatter_df %>%
  ...
  geom_point(colour = nice_colours[["bold_teal_100"]], size = 3) +
  ...
  theme(panel.border = ggplot2::element_rect(color = nice_colours[["black_100"]],
                                             linewidth = 0.3,
                                             fill = NA),
        panel.grid.major.x = ggplot2::element_line(color = nice_colours[["black_25"]])) +
  # Add labels
...
pal <- colorRampPalette((c(nice_colours[["black_25"]], 
                           nice_colours[["bold_teal_100"]])))(5)       # Sequential palette
...
choropleth_chart <- map_df %>% 
  ...
  geom_sf(aes(fill = items_per_1000_bin), color = nice_colours[["white_100"]], lwd = 0.05) +
  # Apply our palette and adjust the legend
  scale_fill_manual(values = pal, 
                    na.value = nice_colours[["black_50"]],
                    guide = guide_legend(
  ...
  geom_sf(data = england_shape_2022, fill = NA, color = nice_colours[["black_100"]], lwd = 0.1) +
  # Add the NICE theme
  nice_gg_theme() +
...
plotly_iris_bar_formatted <- plot_ly(iris_bar_df,
                    ...
                    marker = list(color = nice_colours[["bold_teal_100"]],
                                  line = list(color = nice_colours[['white_100']], width = 0.2)),
...
plotly_hist <- iris %>%
  plot_ly(x = ~Sepal.Width,
          ...
          marker = list(line = list(color = nice_colours[['white_100']], width = 0.4)),
          # Set bin width
          xbins = list(size = 0.2,
                       start = 1.9,
                       end = 4.5),
...
pal <- colorBin("plasma",
                ...
                bins = 5,
                na.color = nice_colours[["black_50"]])
