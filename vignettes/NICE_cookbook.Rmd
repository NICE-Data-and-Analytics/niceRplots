...
Within this cookbook, we will demonstrate how to use the functions included within the `niceRplots` package, and provide examples of plots that have been created in the NICE style. The plots in this cookbook has been created using two graphing packages. The `ggplot2` package is used to create static plots, while the `plotly` package is used to create interactive plots. We have also included an example choropleth map (heatmap) that has been created using the `leaflet` package. Navigate to the relevant section to view the code used to create these charts.
<br>
...
* [Office for National Statistics - Accesibility guidance](https://style.ons.gov.uk/category/accessibility/)
...
* **Alternative text** - This should describe what you want users to understand from the chart. Ideally this should live in the body of the page, directly above or below the chart. In this situation the figure itself should be marked as decorative to be hidden from screen reader software. Refer to the [Office for National Statistics Alt text page](https://style.ons.gov.uk/category/accessibility/alt-text/) for guidance on writing informative alt text. 
...
For guidance on colour usage in charts, please refer to the below links:
...
The `niceRplots` package includes two function to assist with colour usage: 

* `nice_cols()` - used to select individual colours from the NICE brand guidelines e.g. `nice_cols("bold_teal_100")`
* `nice_pal()` - used to select one of five colour palettes e.g. `nice_pal("seq")`. Three primary palettes have been created using the NICE brand colours, and include categorical, sequential and diverging schemes. Secondary categorical and sequential palettes have also been included, using colours recommended by the Government Analysis Function (GAF). These palettes have been visualized below, showing the original colors, and how they look with different color vision deficiencies:
...
colorspace::swatchplot(
  'NICE d ivergent ("div")' = colorRampPalette(nice_pal("div"))(5),
  cvd = c("protan", "deutan", "tritan", "desaturate")
)
colorspace::swatchplot(
...
* **Grid lines**:  grid lines can be added or removed from the axes using the optional `grid_lines` argument (options are `"y"` (default), `"x"`, `"x+y`" and `"none"`). A panel border can be added by setting the `panel_border` argument to `TRUE`. 

* **Axes**: axes ticks can be added or removed using the optional `axis_ticks` argument (options are `"x"` (default), `"y"`, `"x+y`" and `"none"`). Axes can be removed entirely by setting the `remove_axes` argument to `TRUE`. 

* **Legend**: remove the chart legend by setting the `legend` argument to `FALSE`. 
...
map_df <- anticoag_df %>% 
  ...
  # Calculate items_per_100 population, and set up a column with categorical labels for each bin
...
# Set up the sequential colour palette 
...
##### Figure xa: Plotted by sub-ICB location, England 2021
...
# Set up the divergingcolour palette 
...
##### Figure xb: Plotted by sub-ICB location, England 2021
...
It is also possible to create an interactive choropleth map using the `leaflet` package, which provides additional zoom, drag and labeling functionality to allow users to explore the data. 

```{r leaflet, out.width="100%"}

# Using same map data as previous choropleths
