...
Within this cookbook, we will demonstrate how to use the functions included within the `niceRplots` package. We will also provide examples of plots that have been created in the NICE style. The cookbook has been created using two graphing packages. The `ggplot2` package is used to create static plots, while the `plotly` package has been used to create interactive plots. We have also included an example choropleth map (heatmap) that has been created using the `leaflet` package. Navigate to the relevant section to view the code used to create these charts.
<br>

## Installing the niceRplots package

The `niceRplots` is not on CRAN, so you will have to install it directly from [GitHub](https://github.com/) using the `devtools` package.
...
The `niceRplots` package contains 4 functions and 5 data objects. These are summarised below:
...
* `nice_pal()` - used to select primary palettes created using the NICE brand colours, or secondary palettes recommended by the Government Analysis Function.

**Data objects**

* `anticoag_df` - A data frame showing monthly primary care dispensing of 5 anticoagulant medicines for all 106 sub-ICB locations in England between October 2017 and September 2022.
* `sub_icb_shapes_2022` - A data frame containing the April 2022 polygon boundaries for all 106 sub-ICB location in England. 
* `england_shape_2022` - A single polygon containing the boundary for England, created by merging the polygons for all sub_ICB locations.
...
Detailed information on each of these functions and data objects can be found by typing `?name` into the R console e.g. `?nice_gg_theme()` or `?anticoag_df`.
<br>
...
Throughout this cookbook we will use two datasets for our example charts. The first is R's built in `iris` dataset, which you can find information on by typing `?iris` into the R console. The second is the `anticoag_df` dataframe that comes built into the `niceRplots` package. This contains data on the monthly Sub-ICB location level prescribing of 5 anticoagulant medicines in England between February 2017 and January 2022. This was downloaded from [OpenPrescribing.net](https://openprescribing.net/).
...
anticoag_df <- readr::read_csv("example_data.csv",
                            col_types = "Dccdd")
...
Since 2020 all content published on public sector websites must meet the level A and AA success criterion in the [Web Content Accessibility Guidelines (WCAG) 2.1.](https://www.w3.org/TR/WCAG21/), including charts and data visualisations. 

This guide will not go into great detail on the principles of chart accessibility as this has been thoroughly covered elsewhere. Listed below are several useful resources to help design accessible charts. 
...
* **Title** - This should be ten words or less and convey the main message of the chart.

* **Statistical title** - This should have information on what the data is, the geography the data relates to and the time period. This should also include a figure number. 

* **Alternative text** - This should be text shown alongside the chart that describes what you want the user to understand from the chart. Ideally this should live in the body of the page, directly above or below the chart. In this situation the figure itself should be marked as decorative.

* **Source** - If all charts come from the same data source, this does not need to be repeated under each figure. If different data sources are used then you should provide the source in the following format: [organisation] – [publication or source of data]. e.g. NHS Digital - Hospital Episode Statistics.

* **Data download** - It is best practice to provide a way to download the data displayed in each chart, This should be displayed under that chart and include information on file type and size. The text used for the link should be specific to the chart e.g. ‘Download the data for figure 1’.

More detailed information can be found in the [Government Analysis Function - Data visualisation guide](https://analysisfunction.civilservice.gov.uk/policy-store/data-visualisation-charts/).

## Colour usage

Text about color accessibility legislation 

### NICE colours

### NICE Palettes

Five color palettes have been included in the `niceRplots` package, which can be accessed using the `nice_pal()` function. The three primary palettes have been created using the NICE brand colours, and include categorical, sequential and diverging schemes. Secondary categorical and sequential palettes using colours recommended by the Government Analysis Function (GAF) have also been included. These palettes have been visualized below:
...
The `ggplot2` package can be used to create static charts in R. The sections below will demonstrate how to create a range of chart types using ggplot2 and apply the NICE theme.

## How to style your charts

As an example we will create a simple bar chart using the `iris` dataset. We will first create a basic chart, and then add some formatting. We can see the two charts below with and without formatting.

```{r format_chart, fig.show = "hold", out.width = "50%"}
...
iris_bar_formatted <- ggplot(iris_bar_df) + 
  ...
  geom_hline(yintercept = 0, linewidth = 1, colour = nice_cols("black_100")) +
...
* **Colour scheme**: changes line and text colours to be consistent with the NICE brand colour palette. This is particularly noticeable when creating faceted charts (shown later in cookbook). It should be noted that this will not add colour to the plotted data, which will still need to be added manually when creating the chart. For instance, a fill colour was added in the `geom_col()` argument in the above example. 
...
#### **Species setosa has the longest sepal length**{.unlisted .unnumbered}  

##### Figure x: Distribution of sepal width by species

```{r gg_histogram_out, echo = FALSE}
nice_hist
```
<br>
...
##### Figure x: Total anticoagulant medicines dispensed in primary care in England, 2020

```{r gg_bar_out, echo = FALSE}
bar_chart
```
<br>
...
line_chart <- line_df %>%
  ...
  geom_line(linewidth = 0.8, colour = nice_cols("bold_teal_100")) +
  geom_point(size = 1.5, colour = nice_cols("bold_teal_100")) +
...
#### **Edoxaban prescribing has increased since 2017**{.unlisted .unnumbered}  
...
scatter_chart <- scatter_df %>%
  ...
  geom_point(colour = nice_cols("bold_teal_100"), size = 3) +
...
#### **Apixaban prescribing**{.unlisted .unnumbered}  

##### Figure x: Items of apixaban dispensed in primary care compared to sub-ICB location list size, July 2021
...
#### **Trends in anti-coagulant prescribing**{.unlisted .unnumbered}  

##### Figure x: Prescribing of 3 anti-coagulants in England, 2020-2021
...
choropleth_chart <- map_df %>% 
  ...
  geom_sf(aes(fill = items_per_1000_bin), color = nice_cols("white_100"), lwd = 0.05) +
...
#### **Apixaban prescribing in England**{.unlisted .unnumbered}  

##### Figure x: Plotted by sub-ICB location, England 2021

```{r gg_choropleth_out, echo = FALSE}
choropleth_chart
```
<br>
...
**Apixaban was the most prescribed anticoagulant in 2021**  

Figure x. Total anticoagulant medicines dispensed in primary care in England, 2020

```{r plotly_vertical_bar_output, echo = FALSE}
plotly_vbar_chart
```
...
```{r plotly_line_output, echo = FALSE}
plotly_line_chart
```
...
```{r plotly_scatter_output, echo = FALSE}
...
leaflet_map <- map_df %>% 
  ...
  addPolygons(fillColor = ~pal(items_per_1000),
              ...
              label = ~lapply(paste0("<strong>Name: </strong>", map_df$name,
                                     "<br><strong>Items per 1000 people:</strong> ", round(map_df$items_per_1000),2),
                              htmltools::HTML),
              # Set font options
