...
The [Plotly package](https://plotly.com/r/) can be used to create interactive charts in R. Like with `nice_gg_theme()` and ggplot, the `niceRplots` package contains the `nice_plotly_theme()` function which can be applied to apply a NICE theme to a Plotly chart.
...
* **Margins**: adds space between the axis lines and the tick labels. The default Plotly theme has no axis tick marks, just tick labels, and this function adds a gap betweent the axis lines and the tick labels for better spacing. If tick marks are desired, turn the padding off with `pad_axes = FALSE`, otherwise there will be a gap between the tick marks and the axis lines.

* **Modebar**: removes all modebar buttons from the chart, e.g. zoom, select, pan, except for the camera icon allowing the graph to be saved as a PNG.
